...
const RiwayatDiagnosa = () => {
    ...
    const { isPending, isError, data: theDatas, error, isFetching } = useRiwayatDiagnosaPages(limit,page,filter)

    if (isError) return `Error: ${error.message}`

    const handleEdit = (id) => navigate(`/riwayat/diagnosa/edit${id}`)
    const handleDetail = (id) => navigate(`/riwayat/diagnosa/detail/${id}`)
    const handleAdd = () => navigate(`/riwayat/diagnosa/add`)
  return (
    <CRow>
      <CCol xs={12}>
